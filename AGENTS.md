...
- **Hover / tooltip.** Window `pointermove` + screen-space **ellipse** hit test on each bubble’s painted box (3D tilt foreshortens circles); sticky hover keeps the current bubble during peel. Tooltip portaled to `document.body`, anchored via RAF to the hovered bubble’s `getBoundingClientRect()`.
