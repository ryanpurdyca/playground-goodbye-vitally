...
- **Layout engine.** Golden-angle seed + static collision solve to a stable **home** layout (`homeX`/`homeY`, shared `baseR`). DOM bubbles are fixed at home with no hover resize or reposition. Tooltip portaled to `document.body`, anchored to the hovered bubble’s painted `getBoundingClientRect()` (RAF while hovered).
