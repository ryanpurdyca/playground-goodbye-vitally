...
- **`Polaroid` hover / peel** — `Page.tsx` face wrappers are `pointer-events-none`; `Polaroid` uses `pointer-events-auto` for image zoom. Reading-mode peel still comes from `Book.tsx` `pointermove` (left/right by cursor X over the page footprint), not the nav overlay `mouseenter`, so peel works while hovering polaroids too.
...
- **Page 2 (`bookPages[1]`)** — `PolaroidPreview`: three `<Polaroid>`s at upper-left, middle-right, and bottom-left of the page.
