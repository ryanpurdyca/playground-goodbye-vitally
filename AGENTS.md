...
- **Per-page peel**: `Page.tsx` accepts `peeled: boolean` (about-to-flip page base peel), `subPeeled: boolean` (smaller peel on the page directly behind), and `hovered: boolean` (extra peel on hover). A `hoverPeel` MotionValue springs to `±PAGE_BASE_PEEL_DEG (20°)` when peeled, `±PAGE_SUB_PEEL_DEG (10°)` when sub-peeled, or `±PAGE_HOVER_PEEL_DEG (14°)` when hovered. `combinedRotY = useTransform([rotateY, hoverPeel], ([r, h]) => r + h)` adds the peel on top of the base rotateY. Left-stack pages peel positive (toward 0°); right-stack pages peel negative (away from 0°). The 20° base + 10° sub creates a visible two-layer cascade on the right side that geometrically matches the cover-behind-page-0 effect on the left (where the cover at −174° naturally extends further left than the peeled page at −154°).
