       ...
       └─ src/app/layout.tsx        Root layout (Geist, Instrument Serif, Caveat → CSS vars)
            └─ src/app/page.tsx     Server — Stage + frame chrome + book scene
                 └─ Stage           Full-viewport canvas (design system primitive)
                      ...
                      └─ social links (LinkedIn / X / GitHub icons), bottom-right of rule corner
```
...
src/
  app/                       Next.js App Router routes
    ...
    page.tsx                 Home — Stage, gutter, dotted rules, socials, Book
...
- **`page.tsx` frame socials**: LinkedIn / X / GitHub icon links (`public/images/socials/icn-*.svg`), offset from the bottom-right dotted-rule corner (`bottom: 44px`, `right: 52px` = rule inset 28px + 16px / 24px).
...
| Page chrome            | `page.tsx`: 28px gutter, dotted rules at `top/bottom/left/right-7`, social links at `bottom: 44px`, `right: 52px`                                                                                                                          |
...
| Author label           | `Ryan P.` bottom-left of frame (`page.tsx`, `left: 52px`, `bottom: 44px`, always visible; mirrors frame socials)                                                                                                                           |
...
- **Shimmer animation.** A `shimmer` MotionValue is animated `0 → 2π` over 5s, `ease: "linear"`, `repeat: Infinity` via `animate()` in a `useEffect` (controls stopped on unmount). Each wash's centre orbits its anchor by `SHEEN_DRIFT` (≈7%) at a phase offset (`useTransform` with `Math.cos/sin`), so the overlap shifts continuously for a subtle shimmer even when the pointer is still. The sheen layer is always visible.
- **Why a window listener, not element hover.** The cover can't receive its own pointer events — the perspective container sets `pointerEvents: "none"` (see §7). So `Cover` subscribes to `window` `pointermove`, measures `CoverFace`'s projected `getBoundingClientRect()`, and derives a local 0–1 position. A `hover` MotionValue (0/1, spring-smoothed) maps to higher wash alphas plus `brightness(1)` → `brightness(1.14)` on the sheen layer while the cursor is over the cover footprint (always-on base glow, richer colour + slight lift on hover).
- **Springs.** Position (`sheenX/Y`) and hover brightness are smoothed with `COVER_SHEEN_SPRING` (looser than `OPENNESS_SPRING`) so the sheen glides behind the cursor like a reflection. Built into a `background` string with `useMotionTemplate`.
...
- **`highlight-*` popover colors need explicit `@utility` in `globals.css`** — `@theme inline` entries alone do not emit `text-highlight-ink` / `bg-highlight-surface` / `border-highlight-border`; duplicate the token via `@utility` (same pattern as `font-caveat`) or the header falls back to default ink/surface/rule.
...
- **Display page 2 (`bookPages[0]`)** — `ChapterOpen` people cloud (page 1 = inside cover).
- **Display page 3 (`bookPages[1]`)** — `PolaroidPreview`: three offsite `<Polaroid>`s (`img-2026-01` … `03`); `PolaroidPageLabels` (**Autocamp** / **Catskills** / **Offsite**, **Spring** / **2026**).
- **Display page 4 (`bookPages[2]`)** — `TwoPolaroids`: _Flip cup tournament_, _Morning work sessions_, _Axe throwing_ (`img-2026-04` … `06`).
- **Display page 5 (`bookPages[3]`)** — `WinterOffsitePage5`: winter offsite (`img-2026-07` … `09`); `WinterOffsitePageLabels`.
- **Display page 6 (`bookPages[4]`)** — `WinterOffsitePage6`: (`img-2026-10` … `12`).
- **Display pages 7–10 (`bookPages[5]`–`[8]`)** — `NashvilleOffsitePage7` … `NashvilleOffsitePage10`; `NashvilleOffsitePageLabels` on page 7.
...
- **Display pages 19–20 (`bookPages[17]`–`[18]`)** — `NycHolidayOffsitePage19` / `NycHolidayOffsitePage20`; `NycHolidayOffsitePageLabels` on page 19. Page **21** = last-sheet verso (`ThankYouPage` via `BackCoverInsidePage`); page **22** = inside back cover (`BackCover` — `BookPolaroid` “Until next time”, `INSIDE_BACK_COVER_BOOK_PAGE_INDEX`).
...
### 2026-06-03 — Final spread thank-you + bye polaroid; frame socials only

- **`ThankYouPage`** (`src/components/book/ThankYouPage.tsx`) — left-aligned Caveat thank-you on display page 21 (`BackCoverInsidePage` when `bookPages` length is odd).
- **`BackCover`** — centered `BookPolaroid` “Until next time” on the inside back cover (display page 22); interactive via `INSIDE_BACK_COVER_BOOK_PAGE_INDEX` on the final reading spread.
- **`page.tsx`** — removed “Stay in touch” label; social icon links unchanged at bottom-right.
...
- **Position.** `Polaroid` uses plain `useMotionValue` for portal `x`/`y` and `x.set(clientX)` on every pointer move inside the inner photo — no `useSpring(raw)` source tracking. Opacity alone is spring-smoothed.
- **Lightbox gate.** `BookReadingContext` exposes `polaroidLightboxOpen` / `setPolaroidLightboxOpen`; `BookPolaroid` sets it while its lightbox is open so every polaroid on the spread hides the View pill (avoids stuck pill after close without `pointerleave`).
...
- **Chrome stripped on mobile.** `page.tsx`: gutter border and four dotted rules are `hidden md:block`. `LeftPageText` is `hidden md:flex`. **Ryan P.** and social links align to the left/right edges of the closed book (`calc(50vw ± var(--book-width)/2)`), not the frame insets.
