...
- **Hover / tooltip.** Window `pointermove` + screen-space **ellipse** hit test on each bubble’s painted box (3D tilt foreshortens circles); sticky hover keeps the current bubble during peel. Name **Tooltip** above the bubble; **Popover** 8px below (12px placeholder copy) — both portaled to `document.body`, anchored via RAF to `getBoundingClientRect()`. Chrome stays mounted 220ms after hover ends so fade/slide can finish.
